...
class SAFValidationError(Exception):
    """Excepción para detener el cálculo si se violan los límites hidráulicos
    empíricos del cuenco SAF (1.7 <= Fr1 <= 17)."""
    pass
# ------------------------------------------------------------------------------
...
class PantallaInicio(ctk.CTkFrame):
    def __init__(self, master, control_navegacion, ancho_ref=1450, alto_ref=920, **kwargs):
        ...
        ctk.CTkLabel(saf_card, text="Colectores y Azudes",
        ...
        ctk.CTkLabel(footer_ini,
                     ...
                     font=(FUENTE_BASE, 10), text_color="#64748B").pack(expand=True)

# ------------------------------------------------------------------------------
# 6. VISTA: MÓDULO COMPLETO — CUENCO TIPO SAF
# ------------------------------------------------------------------------------
class InterfaceSAF(ctk.CTkFrame):
    ...
    def init_module_ui(self):
        ...
        ctk.CTkLabel(titles_frame,
                     text="Cálculo y dimensionamiento geométrico de cuencos disipadores SAF para un óptimo desempeño hidráulico en obras de derivación y colectores.",
        ...
        ctk.CTkRadioButton(type_frame, text="Colector (Cuenco ancho variable)",
        ...
        footer = ctk.CTkFrame(self, fg_color=COLOR_FONDO_MODULOS, height=50, corner_radius=0,
                               border_width=1, border_color=COLOR_BORDE_TECNICO)
        ...
        ctk.CTkLabel(info_tic,
                     text=f"EPN — FICA  |  Trabajo de Integración Curricular  |  Alex Ante — {datetime.now().strftime('%d/%m/%Y')}",
                     font=(FUENTE_BASE, 9, "bold"), text_color=COLOR_ACCENTO).pack(anchor="w")
    ...
    def create_result_group(self, parent, title, items):
        ...
        for i, item in enumerate(items):
            ...
            ctk.CTkLabel(cell, text=f"{item[0]} [{item[1]}]:",
                         width=100, anchor="w", font=(FUENTE_BASE, 9.5)).pack(side="left")
            ...
            ctk.CTkLabel(cell, text=item[3], width=32, text_color="gray",
                         font=(FUENTE_BASE, 9), anchor="w").pack(side="left")

        if "SAF" in title:
            sf = ctk.CTkFrame(container, fg_color="transparent")
            sf.grid(row=2, column=0, columnspan=3, pady=1, sticky="w")
            ctk.CTkLabel(sf, text="Verificación (1.7 < Fr₁ < 17):",
        ...
        try:
            ...
            if Fr1 < 1.7 or Fr1 > 17.0:
                raise SAFValidationError(
                    f"El número de Froude calculado (Fr₁ = {Fr1:.2f}) está fuera del rango de diseño "
                    f"SAF (Límites: 1.7 ≤ Fr₁ ≤ 17).\n\n"
                    f"Modifique el caudal o la sección para continuar."
                )
            ...
            res_map = {
                ...
                "h6": d2 * (1 + (1 / (3 * C_val))),
                "z_bl": d2 / 3, "Wb3": Wb + 2 * LB, "sigma": sigma
            }

            for k, v in res_map.items():
                if k in self.results:
                    self.results[k].configure(
                        text=f"{v:.2f}" if k == "sigma"
                        else f"{v:.2f}" if k in ["Fr1_ap", "yc", "V1", "C"]
                        else f"{v:.2f}")
...
# ------------------------------------------------------------------------------
# 7. VISTA: MÓDULO COMPLETO — CUENCO TIPO USBR VI
# ------------------------------------------------------------------------------
class InterfaceUSBRVI(ctk.CTkFrame):
    ...
    def init_module_ui(self):
        ...
        btn_frame = ctk.CTkFrame(footer, fg_color="transparent")
        btn_frame.pack(side="right", padx=25, pady=10)
        ctk.CTkButton(btn_frame, text="Calcular", command=self.calcular,
    ...
    def setup_inputs(self):
        ...
        ctk.CTkRadioButton(sel_frame, text="Rectangular", variable=self.tipo_sec, value=1,
                           command=self.toggle_entries,
                           font=(FUENTE_BASE, 11, "bold"), text_color=COLOR_ACCENTO).pack(side="left", padx=10)
        ctk.CTkRadioButton(sel_frame, text="Circular", variable=self.tipo_sec, value=2,
                           command=self.toggle_entries,
                           font=(FUENTE_BASE, 11, "bold"), text_color=COLOR_ACCENTO).pack(side="left", padx=10)
    ...
    def setup_results_table(self):
        ...
        for k, text in [
            ("V", "Velocidad de aproximación  V ≤ 15.24 m/s:"),
            ("Q", "Caudal de diseño de descarga  Q ≤ 11.33 m³/s:"),
            ("Fr", "Número de Froude de aproximación  1.1 ≤ Fr ≤ 10.0:")
        ]:
            f = ctk.CTkFrame(check_container, fg_color="transparent")
            f.pack(fill="x", pady=1)
            ctk.CTkLabel(f, text=text, font=(FUENTE_BASE, 10.5)).pack(side="left")
    ...
    def calcular(self):
        """Mejoras de robustez incorporadas: punto de partida 0.2*D para
        seccion circular (en vez de 0.1*D), factor de amortiguamiento de
        paso 0.5 (en vez de 0.6) y guardia para evitar division por una
        derivada numerica nula en el esquema de Newton-Raphson."""
        ...
        try:
            ...
            if not v_cumple or not q_cumple or not fr_cumple:
                for r in self.results.values(): r.configure(text="0.00")
                self.status_labels["Cav"].configure(text="Diseño Fuera de Rango", text_color="gray")
                self.datos_para_exportar = None
                messagebox.showerror(
                    "Error de Diseño: Límites Superados",
                    f"El diseño NO cumple con los criterios USBR Tipo VI.\n\n"
                    f"Caudal calculado: {Q:.2f} m³/s (Máx: 11.33 m³/s)\n"
                    f"Velocidad calculada: {V_t:.2f} m/s (Máx: 15.24 m/s)\n"
                    f"Número de Froude: {Fr:.2f} (Rango: 1.1 - 10.0)")
                return
    ...
    def exportar(self):
        ...
        try:
            ...
            ws.cell(row=r_curr, column=1, value="Diagnostico Operativo").font = font_regular
            ...
            ws.cell(row=r_curr, column=4, value="Diagnostico").font = font_regular
            for col in range(1, 5): ws.cell(row=r_curr, column=col).border = thin
            ...
            wb.save(path)
            messagebox.showinfo("Exito", "Memoria tecnica exportada correctamente a Excel.")
        except Exception as e:
            messagebox.showerror("Error de Archivo", f"No se pudo guardar.\nDetalle: {e}")

# ------------------------------------------------------------------------------
# 8. CONTROLADOR MAESTRO / NAVEGACIÓN
# ------------------------------------------------------------------------------
