...
class SAFValidationError(Exception):
    """Excepción para detener el cálculo si no cumplen los límites hidráulicos
...
class PantallaInicio(ctk.CTkFrame):
    def __init__(self, master, control_navegacion, ancho_ref=1450, alto_ref=920, **kwargs):
        ...
        ctk.CTkLabel(saf_card, text="Colector y Azud",
...
class InterfaceSAF(ctk.CTkFrame):
    ...
    def toggle_inputs(self):
        ...
        else:  # Cuenco (ancho variable) — sección circular
    ...
    def setup_sections(self):
        ...
        sections = [
            ...
            ("2. Parámetros Hidráulicos", [
                ...
                ("Área Hidráulica", "A",  "A_hid", "m²"),
                ("Ancho Superficial", "T", "T_hid", "m"),
                ("Prof. Hidráulica", "Dₕ", "Dh_hid", "m")]),
    ...
    def create_result_group(self, parent, title, items):
        ...
        if "SAF" in title:
            sf = ctk.CTkFrame(container, fg_color=COLOR_CONTENEDOR, corner_radius=6,
                               border_width=1, border_color=COLOR_BORDE_TECNICO)
            sf.grid(row=0, column=2, padx=3, pady=2, sticky="w")
    ...
    def calcular(self):
        try:
            ...
            if mode == "Derivacion":
                ...
                if y1 <= 0 or y1 >= D0:
                    raise SAFValidationError(
                        f"El tirante y₁ = {y1:.3f} m debe ser positivo y menor que D₀ = {D0:.3f} m.")
                # Geometría sección circular (Chow, 1994)
                r     = D0 / 2.0
                theta = 2.0 * np.arccos((r - y1) / r)          # ángulo central [rad]
                A_c   = (r**2 / 2.0) * (theta - np.sin(theta)) # área mojada [m²]
                T_c   = 2.0 * np.sqrt(max(r**2 - (r - y1)**2, 0.0))  # ancho superficial [m]
                Dh    = A_c / T_c                                # profundidad hidráulica [m]
                V1    = Q / A_c                                  # velocidad media [m/s]
                Fr1   = V1 / np.sqrt(g * Dh)                    # Froude con Dh
                # Ancho del cuenco (HEC-14 Ec. 7-G-1)
            ...
            # Geometría de sección para panel de resultados
            if mode == "Derivacion":
                A_hid  = Br * y1          # sección rectangular
                T_hid  = Br               # ancho superficial = ancho base
                Dh_hid = y1               # prof. hidráulica = tirante (rectangular)
            else:
                A_hid  = A_c              # circular
                T_hid  = T_c
                Dh_hid = Dh
            ...
            res_map = {
                ...
                "A_hid": A_hid, "T_hid": T_hid, "Dh_hid": Dh_hid,
    ...
    def exportar(self):
        ...
        try:
            ...
            nombres_largos = {
                ...
                "t": "Espesor de Pantalla", "Dr": "Diámetro de Roca", "σ": "Índice de Cavitación",
            }
            unidades = {
                "Yn":"m","A":"m2","V":"m/s","Fr":"-","W":"m","L":"m","f":"m","e":"m",
                "H":"m","a":"m","b":"m","c":"m","t":"m","Dr":"m","σ":"-",
            }
